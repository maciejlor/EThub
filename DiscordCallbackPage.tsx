...
import { LoaderIcon, ShieldIcon, XCircleIcon, ClockIcon, SendIcon, CheckCircleIcon, LinkIcon, UserIcon } from 'lucide-react';
...
export function DiscordCallbackPage() {
  ...
  const createJoinRequest = useCallback(async (userData: DiscordUser) => {
    const avatar = userData.avatar
      ? `https://cdn.discordapp.com/avatars/${userData.id}/${userData.avatar}.png`
      : '';

    const alreadyExists = getUsers().some((u) => u.discordId === userData.id);
    ...
    if (!alreadyExists) {
      addUser({
        ...
        isActive: false,
        isPending: true,
        createdBy: 'System (Discord Login)',
      });
    }
  ...
  const handleDiscordCallback = useCallback(async (code: string) => {
    try {
      ...
      handleDiscordCallbackWithToken(tokenData.access_token);
    } catch (err) {
  ...
  const handleDiscordCallbackWithToken = useCallback(async (accessToken: string) => {
    try {
      ...
      const matched =
        matches.find((u) => u.isActive && !u.isPending) ??
      ...
      setMessage('Submitting your join request…');
      await createJoinRequest(userData);
    } catch (err) {
      console.error('Discord OAuth profile fetch error:', err);
      setStatus('error');
      setMessage('Authentication failed. Please try again.');
    }
  }, [createJoinRequest, navigate]);
  ...
  const handleSubmitRequest = async () => {
    ...
    await createJoinRequest(discordUser);
  ...
  const statusConfig: Record<PageStatus, { title: string; icon: React.ReactNode; border: string }> = {
    loading: {
      ...
      icon: <LoaderIcon className="h-8 w-8 animate-spin text-blue-400" />,
    ...
    not_registered: {
      ...
      icon: <ShieldIcon className="h-8 w-8 text-orange-400" />,
          ...
          {status === 'not_registered' && !showLinkAccount && (
            <div className="space-y-3">
              <div className="text-sm text-gray-400 text-center leading-relaxed">
                Your Discord account (<span className="text-white font-mono">{discordUser?.username}</span>) is not registered in EThub.
              </div>
